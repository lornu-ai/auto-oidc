...
import { COMPONENTS, WORKER_CODE } from '@utils/constants'
import { HubType } from '@utils/types'
...
export const App: React.FC = () => {
  const [activeHub, setActiveHub] = React.useState<HubType>(HubType.OIDC)
  const { selectedNodeId, selectNode } = useNodeSelection('worker')
  const { viewMode, setViewMode } = useViewMode('map')
  const { isAnalyzing, aiInsight, analyze } = useAnalyze()
  ...
  const handleAnalyze = async () => {
    ...
    if (!apiKey) {
      alert('VITE_GEMINI_API_KEY environment variable is not set')
      return
    }
    await analyze(apiKey, activeHub)
  }
      ...
      <nav className="relative z-10 border-b border-slate-800 bg-slate-950/50 backdrop-blur-md px-8 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-mono text-sky-400 uppercase tracking-[0.2em]">
              Distributed OIDC Trust Engine
            </p>
          </div>
        </div>
        ...
        <div className="flex bg-slate-900/80 rounded-full p-1 border border-slate-800 gap-1">
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-[10px] font-bold transition-all ${
              viewMode === 'map' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'
            }`}
          >
            TOPOLOGY
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-[10px] font-bold transition-all flex items-center gap-2 ${
              viewMode === 'code' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'
            }`}
          >
      ...
      <main className="relative z-10 flex-1 p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-[1600px] mx-auto w-full">
        <section className="lg:col-span-8 flex flex-col gap-4">
          <div className="flex justify-between items-center px-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setActiveHub(HubType.OIDC)}
                className={`text-[9px] font-bold px-3 py-1 rounded border ${
                  activeHub === HubType.OIDC ? 'bg-white text-black' : 'border-slate-800 text-slate-500'
                }`}
              >
                OIDC
              </button>
              <button
                onClick={() => setActiveHub(HubType.AUTOMATION)}
                className={`text-[9px] font-bold px-3 py-1 rounded border ${
                  activeHub === HubType.AUTOMATION ? 'bg-white text-black' : 'border-slate-800 text-slate-500'
                }`}
              >
                AUTO
              </button>
          ...
          <div className="flex-1 glass rounded-[2.5rem] border border-slate-800 flex items-center justify-center p-8 relative overflow-hidden">
            {viewMode === 'map' ? (
              <svg viewBox="0 0 950 400" className="w-full h-auto drop-shadow-2xl">
                ...
                {nodes.map((node) => (
                  <g
                    ...
                    onClick={() => selectNode(node.id === 'dest' ? 'automation_hub' : node.id)}
                  >
                    <circle
                      cx={node.x}
                      cy={node.y}
                      ...
                      className={`transition-all duration-300 ${
                        selectedNodeId === node.id
                          ? 'fill-sky-500/20 stroke-sky-400 stroke-2'
                          : 'fill-slate-900 stroke-slate-800'
                      }`}
                      filter={selectedNodeId === node.id ? 'url(#glow)' : ''}
                    />
                    <text x={node.x} y={node.y - 5} textAnchor="middle" className="text-xl pointer-events-none">
                      {node.icon}
                    </text>
                    <text
                      x={node.x}
                      y={node.y + 20}
                      textAnchor="middle"
                      className="fill-slate-400 text-[9px] font-bold uppercase tracking-[0.1em] pointer-events-none"
                    >
                      {node.label}
                    </text>
                  </g>
              ...
              <div className="w-full h-full p-8 font-mono text-sm overflow-auto text-sky-300">
                <pre className="animate-in fade-in slide-in-from-left-4 duration-500">{WORKER_CODE}</pre>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass rounded-[2rem] border border-slate-800 p-8">
            <h3 className="text-[10px] font-black text-sky-500 uppercase tracking-widest mb-6">
              Component Deep Dive
            </h3>
            {selectedNode ? (
