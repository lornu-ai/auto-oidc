          ...
          <div className="flex justify-between items-center px-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`text-[9px] font-bold px-3 py-1 rounded border ${activeHub === HubType.OIDC ? "bg-white text-black" : "border-slate-800 text-slate-500"}`}
              >
                OIDC
              </button>
              <button
                onClick={() => setActiveHub(HubType.AUTOMATION)}
                className={`text-[9px] font-bold px-3 py-1 rounded border ${activeHub === HubType.AUTOMATION ? "bg-white text-black" : "border-slate-800 text-slate-500"}`}
              >
                AUTO
              </button>
            </div>
              ...
              <svg viewBox="0 0 950 400" className="w-full h-auto drop-shadow-2xl">
                ...
                {[
                  ...
                  <g
                    ...
                    onClick={() => setSelectedNodeId(n.id === "dest" ? "automation_hub" : n.id)}
                  >
                    <circle
                      cx={n.x}
                      cy={n.y}
                      r="45"
                      className={`transition-all duration-300 ${selectedNodeId === n.id ? "fill-sky-500/20 stroke-sky-400 stroke-2" : "fill-slate-900 stroke-slate-800"}`}
                      filter={selectedNodeId === n.id ? "url(#glow)" : ""}
                    />
                    <text
                      x={n.x}
                      y={n.y - 5}
        ...
        <section className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="glass rounded-[2rem] border border-slate-800 p-8 bg-gradient-to-br from-slate-900 to-black">
            ...
            <div className="text-sm text-slate-300 font-light leading-relaxed max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {aiInsight ||
                "Initiate Gemini-3 Security Review to evaluate the sovereign identity layer."}
            </div>
          </div>
        </section>
      </main>
